...
void IRsend::sendHaipAC(unsigned long data, int nbits)
{
  enableIROut(38);
  mark(HAIP_AC_HDR_MARK);
  space(HAIP_AC_HDR_SPACE);
  for (int i = 0; i < nbits; i++) {
    if (data & TOPBIT) {
      mark(HAIP_AC_BIT_MARK);
      space(HAIP_AC_ONE_SPACE);
    }
    else {
      mark(HAIP_AC_BIT_MARK);
      space(HAIP_AC_ZERO_SPACE);
    }
    data <<= 1;
    if (i == 7) {
      mark(HAIP_AC_BIT_MARK);
