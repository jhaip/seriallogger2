...
def annotationsFetch():
    if request.method == 'POST':
        ...
        if data is None:
            abort(400)
        print data
        tdata = (data["log"], data["timestamp"], data["annotation"], data["value"], data["start_line"], data["start_char"], data["end_line"], data["end_char"])
        ...
        commit_to_db('INSERT INTO annotations (log, timestamp, annotation, value, start_line, start_char, end_line, end_char) VALUES (?,?,?,?,?,?,?,?)', tdata)
        return ('', 204)
    else:
        results = {"results": []}
        keys = ["id", "log", "timestamp", "annotation", "value", "start_line", "start_char", "end_line", "end_char"]
        filter_log = request.args.get('log')
        query = 'select * from annotations'
        query_args = ()
        if filter_log:
            query = 'select * from annotations where log = ?'
            query_args = (filter_log,)
